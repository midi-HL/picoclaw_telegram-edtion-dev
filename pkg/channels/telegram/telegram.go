...
func (c *TelegramChannel) Send(ctx context.Context, msg bus.OutboundMessage) ([]string, error) {
	...
	// Intercept [SEND_STICKER: id] tags from LLM output
	var pendingStickerID string
	re := regexp.MustCompile(`\[SEND_STICKER:\s*([a-zA-Z0-9_-]+)\s*\]`)
	if matches := re.FindStringSubmatch(msg.Content); len(matches) > 1 {
		pendingStickerID = matches[1]
		msg.Content = re.ReplaceAllString(msg.Content, "")
	}

	if msg.Content == "" && pendingStickerID == "" {
		return nil, nil
	}
	...
	// Send sticker if the LLM requested one
	if pendingStickerID != "" {
		c.sendStickerByID(ctx, chatID, threadID, pendingStickerID)
	}
...
func (c *TelegramChannel) appendStickerPrompt(content string) string {
	...
	for _, item := range items {
		...
		if desc == "" {
			desc = "无详细画面描述，请直接参考关联表情与其适用场景进行情感调用"
		}
...
// sendStickerByID reads the sticker store and sends the sticker to the chat.
// If the sticker has a cached TelegramFileID, it sends via FileID (fast).
// Otherwise, it uploads the local file and backfeeds the FileID for future use.
func (c *TelegramChannel) sendStickerByID(ctx context.Context, chatID int64, threadID int, stickerID string) {
	store, err := sticker.NewStickerStore()
	if err != nil {
		logger.WarnCF("telegram", "Failed to open sticker store for sending", map[string]any{
	...
	item, found, err := store.GetByID(stickerID)
	if err != nil || !found {
		logger.WarnCF("telegram", "Sticker not found", map[string]any{
			"sticker_id": stickerID,
		})
		return
	}
	...
	if item.TelegramFileID != "" {
		// Fast path: send via cached FileID
		_, err := c.bot.SendSticker(ctx, &telego.SendStickerParams{
			ChatID:          tu.ID(chatID),
			MessageThreadID: threadID,
			Sticker:         telego.InputFile{FileID: item.TelegramFileID},
		})
		if err != nil {
			logger.ErrorCF("telegram", "Failed to send sticker by FileID", map[string]any{
				"sticker_id": stickerID,
				"error":      err.Error(),
			})
		}
	...
	if item.FilePath == "" {
		logger.WarnCF("telegram", "Sticker has no file path", map[string]any{
			"sticker_id": stickerID,
		})
		return
	}
	...
	if err != nil {
		logger.ErrorCF("telegram", "Failed to open sticker file", map[string]any{
			"sticker_id": stickerID,
			"path":       item.FilePath,
			"error":      err.Error(),
		})
	...
	result, err := c.bot.SendSticker(ctx, &telego.SendStickerParams{
		ChatID:          tu.ID(chatID),
		MessageThreadID: threadID,
		Sticker:         telego.InputFile{File: file},
	})
	if err != nil {
		logger.ErrorCF("telegram", "Failed to send sticker by file upload", map[string]any{
			"sticker_id": stickerID,
			"error":      err.Error(),
		})
		return
	...
	// Backfeed: save the Telegram-assigned FileID for future fast sends
	if result != nil && result.Sticker != nil && result.Sticker.FileID != "" {
		go func() {
			if err := store.BackfeedFileID(stickerID, result.Sticker.FileID); err != nil {
				logger.WarnCF("telegram", "Failed to backfeed sticker FileID", map[string]any{
					"sticker_id": stickerID,
					"error":      err.Error(),
				})
			} else {
				logger.DebugCF("telegram", "Backfed sticker FileID", map[string]any{
					"sticker_id": stickerID,
					"file_id":    result.Sticker.FileID,
				})
			}
		}()
	}
...
func (c *TelegramChannel) handleMessages(ctx context.Context, messages []*telego.Message) error {
	...
	// Inject sticker system prompt so the LLM knows available stickers
	content = c.appendStickerPrompt(content)
...
func (c *TelegramChannel) collectTelegramMessageParts(
	...
	if msg.Sticker != nil {
		...
		if (sticker.IsAnimated || sticker.IsVideo) && sticker.Thumbnail != nil {
			targetFileID = sticker.Thumbnail.FileID
			ext = ".jpg"
		} else {
			targetFileID = sticker.FileID
			ext = ".webp"
		}

		localPath := c.downloadFile(ctx, targetFileID, ext)
		if localPath != "" {
			filename := fmt.Sprintf("sticker-%s%s", sticker.FileID, ext)
			mediaRef := storeMedia(localPath, filename)
			parts.mediaPaths = append(parts.mediaPaths, mediaRef)

			emojiHint := ""
			if sticker.Emoji != "" {
				emojiHint = fmt.Sprintf(" (关联的 Emoji 表情: %s)", sticker.Emoji)
			}
