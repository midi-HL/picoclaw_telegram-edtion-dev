...
import { TelegramStickersManager } from "@/components/channels/channel-forms/telegram-stickers-manager"
...
type TelegramTab = "config" | "stickers"
...
export function TelegramForm({
  ...
  const [activeTab, setActiveTab] = useState<TelegramTab>("config")
  ...
  return (
    <div className="space-y-6">
      {/* Tab bar */}
      <div className="border-border/60 border-b px-1 pt-2">
        <div className="flex gap-8">
          {(["config", "stickers"] as TelegramTab[]).map((tab) => (
            <button
              key={tab}
              type="button"
            ...
            >
              {tab === "config"
                ? t("channels.telegram.tabConfig", "渠道配置")
                : t("channels.telegram.tabStickers", "表情包管理 (Stickers)")}
              {activeTab === tab && (
